...
> **Pre-release (`0.1.0`)** — not yet on PyPI. Install from GitHub:
...
Native OpenTelemetry tracing. Point it at Arize Phoenix, Langfuse, or Jaeger and every retry cycle, failed payload, and Pydantic error shows up as span attributes (`validation.failed`, `retry.{n}.failed_output`, `retry.{n}.error`). Install the extra: `pip install "dspyer[otel] @ git+https://github.com/theramkm/dspyer.git"`. See the [Telemetry guide](#4-observability) / `notebooks/dspyer_playground.ipynb`.
...
Pre-release (`0.1.0`), actively developed. Green CI across Python 3.10–3.14, fully type-checked (`mypy`) and linted (`ruff`), with a 55-case test suite. APIs may shift before `1.0`. Issues and PRs welcome.
